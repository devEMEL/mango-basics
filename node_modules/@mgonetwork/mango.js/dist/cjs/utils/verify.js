...
async function verifyMessage(message, serializedSignature, scope) {
  ...
  if (scope === import_intent.IntentScope.PersonalMessage) {
    const messageBytes2 = (0, import_intent.messageWithIntent)(
      scope,
      import_bcs2.bcs.ser(["vector", "u8"], typeof message === "string" ? (0, import_bcs.fromB64)(message) : message).toBytes()
    );
    if (await signature.pubKey.verify((0, import_sha3.keccak_256)(messageBytes2), signature.signature)) {
      return true;
    }
    const unwrappedMessageBytes = (0, import_intent.messageWithIntent)(
      scope,
      typeof message === "string" ? (0, import_bcs.fromB64)(message) : message
    );
    return signature.pubKey.verify((0, import_sha3.keccak_256)(unwrappedMessageBytes), signature.signature);
  }
  const messageBytes = (0, import_intent.messageWithIntent)(
    scope,
    typeof message === "string" ? (0, import_bcs.fromB64)(message) : message
  );
  return signature.pubKey.verify((0, import_sha3.keccak_256)(messageBytes), signature.signature);
}
