...
const _TransactionBlock = class _TransactionBlock {
  ...
  setGasPayment(payments) {
    __privateGet(this, _blockData).gasConfig.payment = payments.map((payment) => (0, import_superstruct.mask)(payment, import_types.MgoObjectRef));
  }
...
prepareGasPayment_fn = async function(options) {
  ...
  const coins = await expectClient(options).getCoins({
    owner: gasOwner,
    coinType: import_framework.MGO_TYPE_ARG
  });
    ...
    await Promise.all(
      moveModulesToResolve.map(async (moveCall) => {
        ...
        const normalized = await expectClient(options).getNormalizedMoveFunction({
          package: (0, import_mgo_types.normalizeMgoObjectId)(packageId),
          module: moduleName,
