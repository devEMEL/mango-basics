...
__export(multisig_exports, {
  MAX_SIGNER_IN_MULTISIG: () => import_publickey5.MAX_SIGNER_IN_MULTISIG,
  MIN_SIGNER_IN_MULTISIG: () => import_publickey5.MIN_SIGNER_IN_MULTISIG,
...
function toMultiSigAddress(pks, threshold) {
  if (pks.length > import_publickey4.MAX_SIGNER_IN_MULTISIG) {
    throw new Error(`Max number of signers in a multisig is ${import_publickey4.MAX_SIGNER_IN_MULTISIG}`);
  }
  if (pks.length < import_publickey4.MIN_SIGNER_IN_MULTISIG) {
    throw new Error(`Min number of signers in a multisig is ${import_publickey4.MIN_SIGNER_IN_MULTISIG}`);
  }
  let maxLength = 1 + (64 + 1) * import_publickey4.MAX_SIGNER_IN_MULTISIG + 2;
  ...
  return (0, import_mgo_types.normalizeMgoAddress)((0, import_utils.bytesToHex)((0, import_sha3.keccak_256)(tmp.slice(0, i))));
}
function combinePartialSigs(sigs, pks, threshold) {
  if (sigs.length > import_publickey4.MAX_SIGNER_IN_MULTISIG) {
    throw new Error(`Max number of signatures in a multisig is ${import_publickey4.MAX_SIGNER_IN_MULTISIG}`);
  }
  let multisig_pk = {
  ...
  for (let i = 0; i < sigs.length; i++) {
    let parsed = (0, import_utils2.toSingleSignaturePubkeyPair)(sigs[i]);
...
function toPkWeightPair(pair) {
  ...
  switch (pair.pubKey.flag()) {
    case import_signature_scheme.SIGNATURE_SCHEME_TO_FLAG["Secp256k1"]:
      return {
        pubKey: {
          Secp256k1: pk_bytes
        },
    ...
    case import_signature_scheme.SIGNATURE_SCHEME_TO_FLAG["Secp256r1"]:
      return {
        pubKey: {
          Secp256r1: pk_bytes
        },
    ...
    case import_signature_scheme.SIGNATURE_SCHEME_TO_FLAG["ED25519"]:
      return {
        pubKey: {
          ED25519: pk_bytes
        },
