import { fromB64, toB64 } from '@mgonetwork/bcs';
...
export abstract class SignerWithProvider implements Signer {
	 ...
	 */
	get provider(): JsonRpcProvider | MgoClient {
		return this.client;
	}

	readonly client: MgoClient;
	...
	abstract connect(client: MgoClient | JsonRpcProvider): SignerWithProvider;
	 ...
	 */
	async requestMgoFromFaucet(httpHeaders?: HttpHeaders) {
		if (!('requestMgoFromFaucet' in this.provider)) {
			throw new Error('To request MGO from faucet, please use @mgonetwork/mgo.js/faucet instead');
		}

		return this.provider.requestMgoFromFaucet(await this.getAddress(), httpHeaders);
	}

	constructor(client: JsonRpcProvider | MgoClient) {
		this.client = client as MgoClient;
	}
