import { fromB64 } from '@mgonetwork/bcs';
import { is, mask } from 'superstruct';

import type { MgoClient, MgoMoveNormalizedType, ProtocolConfig } from '../client/index.js';
import type { Keypair, SignatureWithBytes } from '../cryptography/index.js';
import { MGO_TYPE_ARG } from '../framework/framework.js';
import type { JsonRpcProvider } from '../providers/json-rpc-provider.js';
import type { MgoObjectResponse } from '../types/index.js';
import {
...
function expectClient(options: BuildOptions): MgoClient {
	...
	return (options.client ?? options.provider!) as MgoClient;
}

const TRANSACTION_BRAND = Symbol.for('@mgonetwork/transaction');
...
interface BuildOptions {
	 ...
	 */
	provider?: JsonRpcProvider | MgoClient;
	client?: MgoClient | JsonRpcProvider;
...
export class TransactionBlock {
	 ...
	 * @deprecated Use `isTransactionBlock` from `@mgonetwork/mgo.js/transactions` instead.
	 */
	 ...
	 * @deprecated Either use the helper methods on the `TransactionBlock` class, or import `Transactions` from `@mgonetwork/mgo.js/transactions`.
	 */
	 ...
	 * * @deprecated Either use the helper methods on the `TransactionBlock` class, or import `Inputs` from `@mgonetwork/mgo.js/transactions`.
	 */
	static get Inputs() {
		return Inputs;
	}
	...
	setGasPayment(payments: MgoObjectRef[]) {
		this.#blockData.gasConfig.payment = payments.map((payment) => mask(payment, MgoObjectRef));
	}
			...
			await Promise.all(
				moveModulesToResolve.map(async (moveCall) => {
					...
					const normalized = await expectClient(options).getNormalizedMoveFunction({
						package: normalizeMgoObjectId(packageId),
						module: moduleName,
		...
		if (objectsToResolve.length) {
			...
			objectsToResolve.forEach(({ id, input, normalizedType }) => {
				...
				if (initialSharedVersion) {
					...
					input.value = Inputs.ObjectRef(getObjectReference(object as MgoObjectResponse)!);
				}
			});
		}
	}
