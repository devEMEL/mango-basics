...
it('can serialize transaction data with a programmable transaction', () => {
	let mgo = normalizeMgoAddress('0x2').replace('0x', '');
	let txData = {
		V1: {
			sender: normalizeMgoAddress('0xBAD').replace('0x', ''),
			...
			kind: {
				ProgrammableTransaction: {
					...
					transactions: [
						{
							kind: 'MoveCall',
							target: `${mgo}::display::new`,
							typeArguments: [`${mgo}::capy::Capy`],
							arguments: [
								// publisher object
								{ kind: 'Input', index: 0 },
							],
						...
						{
							kind: 'MoveCall',
							target: `${mgo}::display::add_multiple`,
							typeArguments: [`${mgo}::capy::Capy`],
							arguments: [
						...
						{
							kind: 'MoveCall',
							target: `${mgo}::display::update_version`,
							typeArguments: [`${mgo}::capy::Capy`],
							arguments: [
